...
Renter.prototype._initMessageBus = function() {
  ...
  var exchangeByteStart = Buffer.from(nodeId, 'hex')[0] - offset;
...
Renter.prototype._handleWork = function(sock, buffer) {
  ...
  if (Renter.SAFE_LANDLORD_METHODS.indexOf(data.method) === -1) {
    ...
    return this.notifier.write(Buffer.from(JSON.stringify({
      id: data.id,
      error: {
        code: -32601,
  ...
  data.params.push(function(err) {
    if (err) {
      return self.notifier.write(Buffer.from(JSON.stringify({
        id: data.id,
        error: {
          code: -32603,
          message: err.message
    ...
    self.notifier.write(Buffer.from(JSON.stringify({
      id: data.id,
      result: args
    })));
  });
...
Renter.prototype._recordResponseTime = function(nodeID,
  ...
  this.storage.models.Contact.findOne({_id: nodeID}, (err, contact) => {
    ...
    if (repPoints > 0) {
      ...
      contact.resetContactErrorCount();
    } else {
      contact.updateContactErrorCount();
    }
...
Renter.prototype._publishContract = function(contacts, contract, callback) {
  ...
  async.each(contacts, (contact, next) => {
    ...
    this.network.publishContract(contactInst, contractInst, (err, data) => {
      ...
      if (err) {
        let errorPoints = points.REQUEST_ERROR;
        if(contact.errorCount > 10 ) {
          errorPoints = errorPoints * 1000
        } else {
          errorPoints = errorPoints * 10
        }
        this._recordResponseTime(nodeID, errorPoints, responseTime);
