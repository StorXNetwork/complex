...
Landlord.prototype._recordRequestTimeout = function(nodeID) {
  this.storage.models.Contact.findOne({_id: nodeID}, (err, contact) => {
    ...
    let errorPoints = points.REQUEST_TIMEOUT;

    // NOTE: Used non round numbers to keep log
    if(contact.errorCount && contact.errorCount > 10) {
      errorPoints = errorPoints * 950
    } else {
      errorPoints = errorPoints * 9
    }

    contact.recordTimeoutFailure();
    contact.updateContactErrorCount();
    contact.recordPoints(errorPoints);
